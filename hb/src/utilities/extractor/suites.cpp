...
void suite8_print_data(){
	...
	int deltat = 1000000, margin = 100000;
	int st = 9000000, end = 9000000 + deltat;
	for (int t0=st;t0<end;t0+=deltat){
		setup_time(t0, t0+deltat+margin);
		long long rdtsc_st = 0, rdtsc_end = 0;
		{
			std::vector<Brick *> bs = ReaderStatic::get().futbase().get("KR4101H60001");
			rdtsc_st = bs[0]->pi.rdtscStamp;
			Brick key;
			key.pi.timestamp = t0+deltat;
			Brick* target = *std::lower_bound(bs.begin(),bs.end(),&key,Functional::ptr_comp<Brick>);
			rdtsc_end = target->pi.rdtscStamp;
		}

		std::vector<Brick *> b1b2;
		{	std::vector<Brick *> bs1 = ReaderStatic::get().futbase().get("KR4101H60001");
			//std::vector<Brick *> bs1_grid = bricks2Grid(bs1,t0,t0+deltat,50000);
			//std::vector<Brick *> bs1_grid = bricks2GridRdtsc(bs1,rdtsc_st,rdtsc_end,50000);
			b1b2.insert(b1b2.end(),bs1.begin(),bs1.end());
			//b1b2.insert(b1b2.end(),bs1_grid.begin(),bs1_grid.end());
		}
		{
			//std::vector<Brick *> bs2 = ReaderStatic::get().callbase().getAll();
			std::vector<Brick *> bs2 = ReaderStatic::get().callbase().get("KR4201H52576");
			//std::vector<Brick *> bs2_grid = bricks2Grid(bs2,t0,t0+deltat,50000);
			//std::vector<Brick *> bs2_grid = bricks2GridRdtsc(bs2,rdtsc_st,rdtsc_end,50000);
			//b1b2.insert(b1b2.end(),bs2_grid.begin(),bs2_grid.end());
			b1b2.insert(b1b2.end(),bs2.begin(),bs2.end());
		}
		std::sort(b1b2.begin(), b1b2.end(), Functional::ptr_comp_func<Brick>(Brick::rdtscComp) );

		std::vector<long long> grids = Functional::makegrid_ll(rdtsc_st, rdtsc_end, 50000);
		std::map<std::string, std::pair<int, double> > strprice;
		int c = 0;
		for (int i=0;i<grids.size();++i){
			while(c < b1b2.size() && grids[i] > b1b2[c]->pi.rdtscStamp){
				int timestamp = b1b2[c]->pi.timestamp;
				double price = b1b2[c]->getValue(Brick::WeightedMidPrice);
				strprice[b1b2[c]->pi.krcodestr] = std::make_pair(timestamp, price);
				++c;
			}
			std::string targets[2] = {"KR4101H60001","KR4201H52576"};
			for (int j=0;j<2;++j){
				std::pair<int, double> p1 = strprice[targets[j]];
				if (p1.first >= t0){
					fprintf(fp,"%lld,%d,%s,%lf\n",grids[i],p1.first, targets[j].c_str(), p1.second);
				}
			}
		}
			/*
